...
_.extend(Err, {
  ...
  E_CONFIG: Err('E_CONFIG', 'no properties match the filter'),
  ...
  async function run_datasource(pageContext, callback) {
    ...
    if (![C.DS_ID_JSON, C.DS_ID_UPTIME, C.DS_ID_TEXT].includes(sieve.datasource_id)) {
      const scrapeClient = new ScrapeClient({
        ...
        filters: config.filters?.included,
        pageContext
      });
      try {
...
class ScrapeClient {
  ...
  constructor({ uri, sieve_id, datasource_id, params, filters, pageContext, pageLoader }) {
    this.uri = uri;
  ...
  async setup() {
    ...
    if (this.filters) {
      pruneSchema(schema, this.filters);
      if (_.isEmpty(schema.properties)) {
        throw new Err.E_CONFIG();
      }
    }
    // create pageLoader
...
/**
 *
 * Copied from @distill/crawler/scraper
 * @param {ContentSchema} schema
 * @param {Array<string>} filters
 *
 * 1. Mark the nodes to keep
 * 2. Remove unmarked nodes
 * 3. Create id2PropertyMap
 */
function pruneSchema(schema, filters) {
  if (filters.some(filter => filter === '.')) {
    return;
  }
  schema._keep = true;
  for (const filter of filters) {
    const filterParts = filter.replace(/"/g, '').split('.').slice(1);
    markSchema(schema, filterParts);
  };
  removeUnmarked(schema);
  // re-create id2PropertyMap
  schema.id2PropertyMap = createId2PropertyMap(schema);
}

/**
  * @param {ContentSchema} schema
  * @param {Array<string>} filterParts
  * set _keep to true on nodes which should be kept
  */
function markSchema(schema, filterParts) {
  let obj = schema;
  let markNodes = [];
  for (let idx = 0; idx < filterParts.length; idx++) {
    const filterPart = filterParts[idx];
    if (filterPart.includes('[]?')) {
      const prop = filterPart.split('[]?')[0];
      if (prop) {
        obj = obj.properties[prop];
      }
    }
    else {
      obj = obj.properties[filterPart];
    }
    if (!obj) {
      return
    }
  ...
  // if the node in the end of filter traversal is a List, then the sub-tree starting from node should be preserved
  if (obj instanceof xlibs.extractorBG.ContentListOfObjectsProperty) {
    obj._keepTree = true;
  }
}

/**
  * @param { ContentSchema | ContentLeafNodeProperty | ContentListOfObjectsProperty}
  */
function removeUnmarked(obj) {
  // return if the sub-tree is to be preserved
  if (obj._keepTree) {
    delete obj._keep;
    delete obj._keepTree
    return;
  } else if (!obj._keep) {
    // remove node from parent.properties
    delete obj.parent.properties[obj.fieldName];
    return;
  }

  delete obj._keep;
  if (obj instanceof xlibs.extractorBG.ContentSchema || obj instanceof xlibs.extractorBG.ContentListOfObjectsProperty) {
    for (const [name, property] of Object.entries(obj.properties)) {
      // TODO: BUG in how schema is created, the property.name is set as tag name
      // which might not be true(if the user edits the name). Setting a field on property
      // manuall for the time being
      property.fieldName = name;
      removeUnmarked(property);
    }
  }
}

function createId2PropertyMap(schema) {
  const id2PropertyMap = new Map();
  (function createMap(node) {
    if (!(node instanceof xlibs.extractorBG.ContentSchema)) {
      id2PropertyMap.set(node.id, node);
    }
